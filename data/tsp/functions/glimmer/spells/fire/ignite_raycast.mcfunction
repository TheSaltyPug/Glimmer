...
execute if block ~ ~ ~ #tsp:glimmer/air unless entity @a[tag=tsp.glimmer.ignite.caster,distance=..1.7] run setblock ~ ~ ~ fire
# damage and set entities on fire
execute at @e[tag=!global.ignore,tag=!global.ignore.kill,tag=!tsp.glimmer.ignite.caster,distance=..1.7] run summon minecraft:small_fireball ~ ~3 ~ {Motion:[0.0,-10.0,0.0]}
